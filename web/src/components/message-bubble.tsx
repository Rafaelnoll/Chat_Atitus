...
export const MessageBubble = ({ message, elementRef }: Props) => {
      ...
      <div className="mb-4" {...(elementRef ? { ref: elementRef } :{})}>
        <div className="flex items-start">
          ...
          <div className="flex-1 ml-2">
            ...
            <div className={`p-3 rounded-2xl ${message.sender === 'You' ? 'bg-indigo-100 dark:bg-indigo-900 ml-auto' : 'bg-gray-100 dark:bg-gray-700'}`}>
              {message.text && <p className="mb-2 text-slate-900">{message.text}</p>}
              {url && (
                <div className="mt-2 text-slate-950">
                  {isImage && (
                    <img src={url} alt={name || 'Image'} className="max-w-full h-auto rounded-lg" />
                  )}
                  {isVideo && (
